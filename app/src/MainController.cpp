...
void MainPlatformEventObserver::on_mouse_move(engine::platform::MousePosition position) {
    ...
    if (!gui_controller->is_enabled()) { camera->rotate_camera(position.dx, position.dy); }
}
...
void MainController::initialize() {
    ...
    // framebuffer inicijalizacija
    m_use_framebuffer = true;
    int screenWidth = platform->window()->width();
    int screenHeight = platform->window()->height();

    auto shader = resources->shader("postprocess_shader");

    graphics->create_postprocess(screenWidth, screenHeight, shader);
...
void MainController::update_camera() {
    ...
    if (platform->key(engine::platform::KeyId::KEY_W).is_down()) { camera->move_camera(engine::graphics::Camera::Movement::FORWARD, 4 * dt); }
    if (platform->key(engine::platform::KeyId::KEY_S).is_down()) { camera->move_camera(engine::graphics::Camera::Movement::BACKWARD, 4 * dt); }
    if (platform->key(engine::platform::KeyId::KEY_A).is_down()) { camera->move_camera(engine::graphics::Camera::Movement::LEFT, 4 * dt); }
    if (platform->key(engine::platform::KeyId::KEY_D).is_down()) { camera->move_camera(engine::graphics::Camera::Movement::RIGHT, 4 * dt); }
}
...
void MainController::update() {
    ...
    // Zemlja rotacija
    if (platform->key(engine::platform::KeyId::KEY_1).state() == engine::platform::Key::State::JustPressed) { m_earth_rotation_enabled = !m_earth_rotation_enabled; }
    // Zemlja rotacija oko Sunca
    if (platform->key(engine::platform::KeyId::KEY_2).state() == engine::platform::Key::State::JustPressed) { m_earth_orbit_enabled = !m_earth_orbit_enabled; }
    // Sunce rotacija
    if (platform->key(engine::platform::KeyId::KEY_3).state() == engine::platform::Key::State::JustPressed) { m_sun_rotation_enabled = !m_sun_rotation_enabled; }
    // Mesec rotacija
    if (platform->key(engine::platform::KeyId::KEY_4).state() == engine::platform::Key::State::JustPressed) { m_moon_rotation_enabled = !m_moon_rotation_enabled; }
    // Mesec rotacija oko Zemlje
    if (platform->key(engine::platform::KeyId::KEY_5).state() == engine::platform::Key::State::JustPressed) { m_moon_orbit_enabled = !m_moon_orbit_enabled; }

    // event
    if (platform->key(engine::platform::KeyId::KEY_T).state() == engine::platform::Key::State::JustPressed) { start_timed_event(3.0, 6.0); }
    // reset event
    if (platform->key(engine::platform::KeyId::KEY_R).state() == engine::platform::Key::State::JustPressed) { reset_timed_event(); }
    ...
    if (m_earth_rotation_enabled) {
        ...
        if (m_earth_rotation_angle > glm::two_pi<float>()) m_earth_rotation_angle -= glm::two_pi<float>();
    }

    // Rotacija Zemlje oko Sunca
    if (m_earth_orbit_enabled) {
        m_earth_orbit_angle += dt * glm::radians(5.0f);
        if (m_earth_orbit_angle > glm::two_pi<float>()) m_earth_orbit_angle -= glm::two_pi<float>();
    }
    ...
    if (m_sun_rotation_enabled) {
        ...
        if (m_sun_rotation_angle > glm::two_pi<float>()) m_sun_rotation_angle -= glm::two_pi<float>();
    }

    // Rotacija Meseca oko svoje ose
    if (m_moon_rotation_enabled) {
        m_moon_rotation_angle += dt * glm::radians(80.0f);
        if (m_moon_rotation_angle > glm::two_pi<float>()) m_moon_rotation_angle -= glm::two_pi<float>();
    }

    // Rotacija Meseca oko Zemlje
    if (m_moon_orbit_enabled) {
        m_moon_orbit_angle += dt * glm::radians(50.0f);
        if (m_moon_orbit_angle > glm::two_pi<float>()) m_moon_orbit_angle -= glm::two_pi<float>();
    }
...
void MainController::begin_draw() {
    auto platform = engine::core::Controller::get<engine::platform::PlatformController>();
    m_post_process->begin_draw(platform->window()->width(), platform->window()->height());

    engine::graphics::OpenGL::clear_buffers();
}

void MainController::resize_framebuffer(int width, int height) { m_post_process->resize(width, height); }

void MainController::draw() {
    draw_sun();
    draw_earth();
    if (m_draw_moon) { draw_moon(); }
    draw_skybox();
...
glm::vec3 MainController::get_earths_position() const { return m_earth_position; }

glm::vec3 MainController::get_moons_position() const { return m_moon_position; }

bool MainController::is_earth_rotation_enabled() const { return m_earth_rotation_enabled; }

bool MainController::is_earth_orbit_enabled() const { return m_earth_orbit_enabled; }

bool MainController::is_sun_rotation_enabled() const { return m_sun_rotation_enabled; }

bool MainController::is_moon_rotation_enabled() const { return m_moon_rotation_enabled; }

bool MainController::is_moon_orbit_enabled() const { return m_moon_orbit_enabled; }

bool MainController::is_moon_visible() const { return m_draw_moon; }

bool MainController::is_event_active() const { return m_timed_event.active; }